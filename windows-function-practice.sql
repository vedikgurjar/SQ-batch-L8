...
select department, sum(sales_amount) from employee_performance;-- error
...
select employee_id, employee_name, location, sales_amount,
sum(sales_amount) over(partition by location order by sales_amount) from employee_performance;

create table employ_6 as select employee_id
employee_name, location, round(sales_amount/100) as amount
from employee_performance limit 5;

select *, sum(amount) over() from employ_6;
select *, sum(amount) over (partition by location)
from employ_6;
...
select *, sum(amount) over(order by amount) from employ_6;
select *, sum(amount) over(order by location) from employ_6;
select *, sum(amount) over(order by employee_name) from employ_6;

select *, sum(amount) over(partition by location order by amount) from employ_6;