...
-- rank ranking dega per uske pass compare karne  ke liye koi value ho warna sab pr 1 dikhaega
select category, order_id, amount,
rank() over(partition by category order by amount desc) from orders;

-- find the difference betweeen each order amount and the average amount of its category
select order_id, category, amount,
avg(amount) over(partition by category),
round(amount - avg(amount) over(partition by category)) as difference
from orders;
...
max(amount) over(partition by customer_id order by amount desc) as customer_max
from orders) as t
...
-- find th two highest value orders for every customer

select * from 
(select order_id, customer_id, customer_name, amount,
row_number() over (partition by customer_id order by amount desc) as highest_value
from orders) as t
where highest_value <= 2;

-- calculate the percentage contibution of each order to the customer's total spending
